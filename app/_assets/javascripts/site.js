      ...
      if (responsiveSettings && responsiveSettings.length > -1) {
        ...
        for (breakpoint in responsiveSettings) {
          if (responsiveSettings.hasOwnProperty(breakpoint)) {
            ...
            _.breakpointSettings[responsiveSettings[breakpoint].breakpoint] =
...
var site = {
  globals: {
    sitename: "Bad Credit New Car!",
  },
};

// reusable ready function (abstracted so that ready lib can be agnostic, jQuery by default)
site.onReady = function (func) {
  if (func) {
    ...
    console.log("You didn't pass site.onReady a function to fire!");
  }
};

site.initFastclick = function () {
  // add fastclick
  $j(function () {
...
site.handleMenu = {
  config: {
    ...
    togglerSel: ".js-menu-toggler",
    menuSel: ".js-menu-object",
  },

  init: function () {
    var self = this;
    var $menu = $j(self.config.menuSel);
    var $toggler = $j(self.config.togglerSel);

    if ($menu.length) {
      $toggler.on("click", function (e) {
        $toggler.toggleClass(self.config.activeClass);
        $menu.toggleClass(self.config.activeClass);
      });
    }
  },
};

site.handleToggleItems = {
  config: {
    toggleItemSel: ".js-toggle-item",
    hiddenClass: "is--hidden",
  },

  init: function () {
    var self = this;
    var $toggleItems = $j(self.config.toggleItemSel);
    var hiddenClass = self.config.hiddenClass;

    if ($toggleItems.length) {
      $toggleItems.each(function (index) {
        var $thisToggleItem = $j(this);
        var thisData = $thisToggleItem.data();

        if (thisData.togglerItem) {
          var $thisToggleTarget = $j(thisData.togglerItem);

          if ($thisToggleTarget.length) {
            $thisToggleItem.on("click", function (e) {
              e.preventDefault();

              if ($thisToggleTarget.hasClass(hiddenClass)) {
                $thisToggleTarget.removeClass(hiddenClass);
                site.handleScrollToLinks.scrollTo($thisToggleTarget);
              } else {
                $thisToggleTarget.addClass(hiddenClass);
              }
            });
          }
        }
...
site.handleScrollToLinks = {
  config: {
    scrollToTriggers: "[data-scroll-to]",
  },
  ...
  init: function () {
    ...
    var jScrollToTriggers = $j(self.config.scrollToTriggers);

    if (jScrollToTriggers.length) {
      jScrollToTriggers.each(function (index) {
        var jThisTrigger = $j(this);
        var thisTarget = jThisTrigger.data().scrollTo;

        if (thisTarget !== undefined) {
          jThisTrigger.on("click", function (e) {
            e.preventDefault();
            self.scrollTo(thisTarget);
          });
        }
      });
    }
...
site.handleTogglers = {
  config: {
    toggleTrigger: "is--toggler",
    hiddenClass: "is--hidden",
  },

  init: function () {
    var self = this;
    var jToggleTriggers = $j("." + self.config.toggleTrigger);

    if (jToggleTriggers.length) {
      jToggleTriggers.each(function () {
        var jThisToggler = $j(this);
        var jThisData = jThisToggler.data();

        jThisToggler.on("click", function (e) {
          var jThisToggleTarget = $j(jThisData.toggleTarget);

          if (jThisToggleTarget.length) {
            e.preventDefault();

            if (jThisData.hideAll) {
              $j(jThisData.hideAll).addClass(self.config.hiddenClass);
            }

            if (jThisToggleTarget.hasClass(self.config.hiddenClass)) {
              jThisToggleTarget.removeClass(self.config.hiddenClass);
            } else {
              jThisToggleTarget.addClass(self.config.hiddenClass);
            }
          } else {
            console.log("Cant' find " + jThisData.toggleTarget);
          }
        });
      });
    }
...
site.handlePickers = {
  config: {
    ...
    timepicker: "is--timepicker",
  },

  handleDates: function (jDatePickers) {
    var self = this;

    jDatePickers.each(function (index) {
      var jThisPicker = $j(this);

      jThisPicker.pickadate({
  ...
  handleTimes: function (jTimePickers) {
    var self = this;

    jTimePickers.each(function () {
      var jThisPicker = $j(this);

      jThisPicker.pickatime({
        min: [8, 30],
        max: [20, 0],
      });
    });
  ...
  init: function () {
    var self = this;
    var jDatePickers = $j("." + self.config.datepicker);
    var jTimePickers = $j("." + self.config.timepicker);

    if (jDatePickers.length) {
      self.handleDates(jDatePickers);
    }

    if (jTimePickers.length) {
      self.handleTimes(jTimePickers);
    }
  },
};
...
site.handleForms = {
  config: {
    form: "is--validated",
    frow: "frow.is--required",
    field: "frow__field",
    frowErrorClass: "has--error",
    frowErrorMsg: "frow__error",
  },

  validateForm: function (jThisForm) {
    var self = this;
    var jFrows = jThisForm.find("." + self.config.frow);
    var hasErrors = false;

    if (jFrows.length) {
      jFrows.each(function () {
        var jThisFrow = $j(this);
        jThisFrow.removeClass(self.config.frowErrorClass);
        var jThisErrorDiv = jThisFrow.find("." + self.config.frowErrorMsg);

        var thisErrorMsg = "This is a required field";

        var thisField = jThisFrow.find("." + self.config.field).first();
        var thisFieldVal = thisField.val();
        var thisFrowData = jThisFrow.data();

        if (thisFrowData.validationMsg) {
          thisErrorMsg = thisFrowData.validationMsg;
        }

        if (thisFieldVal == "") {
          hasErrors = true;

          jThisFrow.addClass(self.config.frowErrorClass);

          if (jThisErrorDiv.length == 0) {
            thisErrorDiv = $j('<div class="frow__error"></div>').text(
              thisErrorMsg
            );
            jThisFrow.append(thisErrorDiv);
          }

          thisField.on("change keydown keyup", function (e) {
            jThisFrow.removeClass(self.config.frowErrorClass);
          });
        }
      });
  ...
  handleForm: function (jForms) {
    var self = this;

    jForms.each(function () {
      var jThisForm = $j(this);
      var jThisSubmit = jThisForm.find(".form__submit");

      jThisSubmit.on("click", function (e) {
        var isValid = self.validateForm(jThisForm);
        return isValid;
      });
    });
  },
  ...
  init: function () {
    ...
    var jForms = $j("." + self.config.form);

    if (jForms.length) {
      self.handleForm(jForms);
    }
  },
};

site.handleCarousels = {
  config: {
    carouselSel: ".carousel",
  },

  genArrow: function (modifier, text, icon) {
    return (
      '<button class="carousel-control carousel-control--' +
      modifier +
      '"><span class="carousel-control__icon icon-' +
      icon +
      '"></span><span class="carousel-control__text">' +
      text +
      "</span></button>"
    );
  },
  ...
  init: function () {
    var self = this;
    var $carousels = $j(self.config.carouselSel);

    if ($carousels.length) {
      $carousels.slick({
        prevArrow: self.genArrow("prev", "Previous", "prev"),
        nextArrow:
          '<button class="carousel-control carousel-control--' +
          "next" +
          '"><span class="carousel-control__text">' +
          "Next" +
          '</span><span class="carousel-control__icon icon-' +
          "next" +
          '"></span></button>',
        responsive: [
          {
...
site.init = function () {
  site.initFastclick();
  site.handleMenu.init();
  site.handleCarousels.init();
  site.handleTogglers.init();
  site.handleScrollToLinks.init();
  site.handlePickers.init();
  site.handleForms.init();
  site.handleToggleItems.init();
};

// on DOM ready run the following:
